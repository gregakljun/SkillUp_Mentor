...
export class AbstractService {
  ...
  async findById(id: string, relations = []): Promise<any> {
    try {
      ...
      Logging.error(error)
      throw new InternalServerErrorException(`Something went wrong while searching for an element with and id: ${id}`)
    }
  }
  ...
  async remove(id: string): Promise<any> {
    ...
    try {
      ...
      Logging.error(error)
      throw new InternalServerErrorException('Something went wrong while deliting an element')
    }
  }
