...
export class OrdersService extends AbstractService {
    ...
    async export(response: Response): Promise<any> {
        ...
        const orders = await this.findAll(['order_items'])
        orders.forEach((o) => {
            json.push({
                ID: o.id,
                Name: o.name,
    ...
    async chart(): Promise<{ date: string; sum: string }[]> {
        const apiData = await this.orderRepository.query(`
            SELECT to_date(cast(o.created_at as TEXT), '%Y-%m-%d') as date, sum(oi.price * oi.quantity) as sum FROM "orders" o
        ...
        for (let index = 0; index < apiData.length; index++) {
            chartData.push({
                date: (new Date(apiData[index].date)).toISOString().split('T')[0],
